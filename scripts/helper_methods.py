...
def reset_client_window(debug = True):
    clint_pos = imagesearch('images/GG_icon3_dark.png', precision=0.90, debug=debug)
    if clint_pos != [-1, -1]:
        while True:
            ...
            pyautogui.mouseUp()
            time.sleep(.3)
            clint_pos = imagesearch('images/GG_icon3_dark.png', precision=0.9, debug=debug)
            if clint_pos == def_clint:
                return True
            # time.sleep(2)
    else:
...
def login():
    ...
    if compare_img_screenshot(upper_corner,(420, 148)):
        ...
        time.sleep(2)
        upper_corner_pos = imagesearch('images/login_popup_upper_corner.png', precision=0.80)    
